...
class MyString {
	...
	String str;
	char[] s = new char[50]; // 문자 저장 공간
	int length;              // 실제 저장된 문자열 길이 추적
	
	// 생성자: 초기 문자열을 받아 배열에 문자를 하나씩 채움
	public MyString(String str) {
		this.str = str;
		this.length = str.length();
		for(int i = 0; i < length; i++) // 문자열 길이만큼 반복하여 char[] s 에 하나씩 복사
			s[i] = str.charAt(i);
	}
	...
	public String toString() {
		// 생성 char[] 배열의 0번 인덱스부터 length 만큼만 문자열로 변환
		return new String(s, 0, length);
	}
	
	// 현재 문자열 뒤에 새로운 문자열을 이어붙이는 함수
	public String concat(String addStr) {
		// 기존 길이 + 붙일 길이 만큼 새로운 배열 생성
		char[] newArr = new char[this.length + addStr.length()];
		// 기존 문자를 새 배열의 앞부분에 복사
		for(int i = 0; i < length; i++) 
			newArr[i] = this.s[i];
		// 붙일 문자를 새 배열의 뒷부분에 복사
		for (int i = 0; i < addStr.length(); i++) {
			newArr[this.length + i] = addStr.charAt(i);
		}
	...
	public String trim() {
		int start = 0;
		int end = length - 1;
		// 앞에서부터 공백이 아닌 문자가 나올 때까지 인덱스 이동
		while (start <= end && s[start] <= ' ') {
			start++;
		}
		// 뒤에서부터 공백이 아닌 문자가 나올 때까지 인덱스 이동
		while (end >= start && s[end] <= ' ') {
			end--;
		}
		// 모든 문자가 공백이라면, 빈 문자열 반환
		if (start > end) return "";
		// 시작과 끝 사이 찾아낸 문자들만 뽑아서 새 문자열 생성 후 리턴
		return new String(s, start, end - start + 1);
	}
	
	// 특정 문자를 찾아 새로운 문자로 모두 바꾸는 함수
	public String replace(char oldChar, char newChar) {
		// 동일한 크기의 새 배열을 생성하여 복사본으로 작업 수행(원본 훼손 방지)
		char[] newArr = new char[length];
		for (int i = 0;i < length; i++) {
			// 현재 위치의 문자가 바꾸려는 문자와 같다면 newChar 로 넣음
			if (s[i] == oldChar) {
				newArr[i] = newChar;
			} else { // 다르면 원래 문자 그대로 복사
				newArr[i] = s[i];
			}
		}
		return new String(newArr);
	...
	// 특정 구분자를 기준으로 문자열을 잘라 배열로 반환하는 함수
	public String[] split(char delimiter) {
		int count = 1;
		// 구분자가 몇 개 있는지 확인해서 결과 배열 크기 결정
		for (int i = 0; i < length; i++) {
			if (s[i] == delimiter) count++;
		}
		
		String[] result = new String[count];
		String temp = ""; // 글자를 하나씩 이어 붙일 임시 변수
		int idx = 0;
		
		for (int i = 0; i < length; i++) {
			// 구분자를 만났을 때,
			if (s[i] == delimiter) {
				result[idx++] = temp; // 지금까지 모은 글자를 배열에 저장
				temp = "";
			}else { // 구분자가 아니면 문자를 temp 에 누적
				temp += s[i];
			}
		}
		// 반복문 종료 후, 마지막 남은 조각을 배열의 마지막 칸에 저장
		result[idx] = temp;
		return result;
	}
	
	// 시작 인덱스부터 끝 인덱스 직전까지의 문자열을 추출하는 함수
	public String substring(int beginIndex, int endIndex) {
		// 잘못된 범위가 들어올 경우를 방지하는 예외 처리 코드
...
public class Test {
	...
	public static void main(String[] args) {
		MyString myStr = new MyString("  Hello Java  ");
		
		System.out.println("Trim: [" + myStr.trim() + "]");
		
		System.out.println("Concat: " + myStr.trim().concat(" Study"));
		
		MyString myStr2 = new MyString("apple");
		System.out.println("Replace: " + myStr2.replace('p', 'b'));
		
		System.out.println("Substring: " + myStr2.substring(0,  3));
		
		MyString myStr3 = new MyString("Red,Green,Blue");
		String[] colors = myStr3.split(',');
...
→ 기본 타입은 Wrapper 객체로 만들어 저장하고, 객체, null 등 삽입
...
→ iterator() 메소드는 Iterator 객체를 반환함
...
→ 모든 메소드는 static 타입
→ 주요 메소드: sort(), reverse(), max(), min(), binarySearch()

---

제네릭

- 구체화: 제네릭 타입 클래스에 구체적인 타입을 대입하여 객체 생성
              컴파일러에 의해 이루어진다.
- 장점: 컴파일 시 타입이 결정되어 보다 안전함
          런타임 타입 충돌 문제 방지
...
입출력 스트림 특징
→ 단방향: 입력과 출력을 동시에 하는 스트림은 없다
→ 선입선출 구조를 가진다.

문자 스트림
→ 유니 코드(2byte) 문자를 입출력
→ 문자 데이터만 다루며, 바이너리 데이터 입출력 불가
→ Reader/Writer 계열 클래스가 문자 스트림을 다룸

```java
// 텍스트 파일을 읽어서 터미널창에 출력하는 코드
package example_07;
