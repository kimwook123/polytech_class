...
def main():
    ...
    best_model_path = r"C:\Users\AISW_203_110\Desktop\train_mini\runs\detect\ppe_detection-2\weights\best.pt"
    model = YOLO(best_model_path) 

    # 3. YAML 파일 경로 설정 (경로 문자열 앞에 r 붙이기)
    yaml_path = r"C:\Users\AISW_203_110\Desktop\train_mini\data.yaml"
    
    print("🚀 기존 지식을 바탕으로 미착용자 데이터를 집중 추가 학습합니다...")
    ...
    results = model.train(
        ...
        imgsz=640,              # 이미지 크기 설정
        batch=-1,               # 16~32 추천 (3080 메모리에 맞춰 조절)
        device=0,               # 0번 GPU 사용
        name="ppe_detection_finetune",   # 결과가 저장될 폴더 이름 (runs/detect/ppe_detection)
        workers=8,              # 데이터 로딩 스레드 개수
        patience=10,            # (추가) 20 Epoch 동안 성능 향상이 없으면 조기 종료 (과적합 방지)
        save=True               # (추가) 학습 완료 후 최고 성능의 가중치(best.pt) 저장
    )
    
    print("🎉 파인튜닝 학습이 완료되었습니다! runs/detect/ppe_detection/weights/best.pt 파일을 확인하세요.")
